...
use soth_mitm::test_engine::{CompatibilityOverrideConfig, MitmConfig, MitmEngine, TlsProfile, UpstreamSniMode};
...
async fn strict_profile_fails_tls12_upstream_with_handshake_taxonomy() {
    ...
    let _ = tls
        .write_all(b"GET /tls12-strict HTTP/1.1\r\nHost: 127.0.0.1\r\nConnection: close\r\n\r\n")
        .await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn compat_profile_succeeds_against_tls12_upstream_fixture() {
    let upstream_listener = TcpListener::bind("127.0.0.1:0")
        .await
        .expect("bind upstream listener");
    let upstream_addr = upstream_listener.local_addr().expect("upstream addr");
    let upstream_task = tokio::spawn(async move {
        ...
        assert!(
            request_text.starts_with("GET /tls12-compat HTTP/1.1"),
            "{request_text}"
        );
    ...
    let mut tls = connector
        ...
        .await
        .expect("TLS connect to sidecar");
    tls.write_all(b"GET /tls12-compat HTTP/1.1\r\nHost: 127.0.0.1\r\nConnection: close\r\n\r\n")
        .await
