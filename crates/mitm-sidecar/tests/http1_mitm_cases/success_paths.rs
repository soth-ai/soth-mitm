...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn forward_http10_absolute_form_request_relays_without_connect() {
    let upstream_listener = TcpListener::bind("127.0.0.1:0")
        .await
        .expect("bind upstream listener");
    let upstream_addr = upstream_listener.local_addr().expect("upstream addr");
    let upstream_task = tokio::spawn(async move {
        ...
        assert!(
            request_line.starts_with("GET /legacy-http10 HTTP/"),
            "{request_text}"
        );
    ...
    let mut client = TcpStream::connect(proxy_addr)
        ...
        .expect("connect sidecar");
    let request = format!(
        "GET http://127.0.0.1:{}/legacy-http10 HTTP/1.0\r\nConnection: close\r\n\r\n",
        upstream_addr.port()
    );
    client
        .write_all(request.as_bytes())
    ...
    assert!(response_text.ends_with("legacy"), "{response_text}");
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn intercept_http11_request_hooks_receive_ai_host_header_for_capture() {
    let upstream_listener = TcpListener::bind("127.0.0.1:0")
        .await
        .expect("bind upstream listener");
    let upstream_addr = upstream_listener.local_addr().expect("upstream addr");
    let upstream_task = tokio::spawn(async move {
        ...
        let request_text = String::from_utf8_lossy(&request_head);
        assert!(
            request_text.starts_with("GET /v1/models HTTP/1.1"),
            "{request_text}"
        );
        let response = b"HTTP/1.1 200 OK\r\nContent-Length: 2\r\nConnection: close\r\n\r\nok";
        tls.write_all(response).await.expect("write response");
    ...
    let events = sink.snapshot();
    assert!(events.iter().any(|event| {
        event.kind == EventType::RequestHeaders && event.context.server_host == "api.openai.com"
    }));
}
