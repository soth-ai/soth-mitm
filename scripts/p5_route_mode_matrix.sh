...
run_case route_config_validation \
  cargo test --all-features -p soth-mitm route_mode_requires_corresponding_endpoint -q
run_case route_config_rejects_unexpected_proxy_fields \
  cargo test --all-features -p soth-mitm route_mode_rejects_unexpected_endpoint -q
run_case route_planner_unit_matrix \
  cargo test --all-features -p soth-mitm route_planner_ -q
run_case route_mode_integration_matrix \
  cargo test --all-features -p soth-mitm --test route_mode_matrix -q
