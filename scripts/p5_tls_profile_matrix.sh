...
run_case profile_schema_guardrails \
  cargo test --all-features -p soth-mitm strict_tls_profile_rejects_disabled_sni_mode -q
run_case tls_profile_client_matrix \
  cargo test --all-features -p soth-mitm strict_profile_builds_tls13_only_client_with_required_sni -q
run_case tls_profile_cipher_matrix \
  cargo test --all-features -p soth-mitm compat_profile_reorders_cipher_suites_for_tls12_first -q
run_case cert_profile_fixture_chain \
  cargo test --all-features -p soth-mitm cert_store_compat_profile_issues_compatible_leaf_public_keys -q
run_case tls_profile_handshake_matrix \
  cargo test --all-features -p soth-mitm --test tls_profile_matrix -q
