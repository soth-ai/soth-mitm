...
: >"$status_tsv"
run_case core_conformance cargo test --all-features -p soth-mitm --test conformance_golden -q
run_case mitmproxy_tls_taxonomy cargo test --all-features -p soth-mitm --test mitmproxy_tls_adapter -q
run_case tls_learning_guardrails cargo test --all-features -p soth-mitm --test tls_learning_guardrails -q
