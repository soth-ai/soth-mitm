...

{
  run_case http1_parser_api_smuggling_guards \
    cargo test --all-features -p soth-mitm http_head_parser_api_tests -q
  run_case http1_smuggling_fixture_corpus \
    cargo test --all-features -p soth-mitm --test http1_head_corpus -q
  run_case http1_forward_absolute_form_proxy_semantics \
    cargo test --all-features -p soth-mitm --test http1_mitm \
