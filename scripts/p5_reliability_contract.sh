...

{
  run_case flow_fsm_transition_validator \
    cargo test --features soth-mitm/__internal -p soth-mitm invalid_transition_terminalizes_without_panic_and_allows_close -q
  run_case single_stream_closed_guardrail \
    cargo test --features soth-mitm/__internal -p soth-mitm suppresses_duplicate_stream_closed_for_same_flow -q
  run_case runtime_budget_metrics \
    cargo test --features soth-mitm/__internal -p soth-mitm --test runtime_governor \
