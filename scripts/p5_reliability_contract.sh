...

{
  run_case flow_fsm_transition_validator \
    cargo test --all-features -p soth-mitm invalid_transition_terminalizes_without_panic_and_allows_close -q
  run_case single_stream_closed_guardrail \
    cargo test --all-features -p soth-mitm suppresses_duplicate_stream_closed_for_same_flow -q
  run_case runtime_budget_metrics \
    cargo test --all-features -p soth-mitm --test runtime_governor \
  ...
  run_case h2_stage_budget_metrics \
    cargo test --all-features -p soth-mitm --test mixed_traffic_soak \
      tls_h2_exchange_harness_path_succeeds -q
  run_case codex_intercept_terminalization_contract \
    cargo test --all-features -p soth-mitm --test http2_mitm \
