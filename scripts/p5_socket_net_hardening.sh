...
run_case sidecar_socket_hardening_ipv6 \
  cargo test --features soth-mitm/__internal -p soth-mitm --test socket_hardening -q
run_case route_matrix_socket_path \
  cargo test --features soth-mitm/__internal -p soth-mitm --test route_mode_matrix upstream_socks5_mode_honors_ignore_host_and_relays_tunnel -q
