...
run_case unknown_ca_native_path \
  cargo test --features soth-mitm/__internal -p soth-mitm --test http1_mitm intercept_upstream_tls_failure_emits_taxonomy_reason -q
run_case timeout_reset_invalid_chain_authoritative_path \
  cargo test --features soth-mitm/__internal -p soth-mitm --test mitmproxy_tls_adapter replayed_mitmproxy_failed_callbacks_match_native_taxonomy -q
