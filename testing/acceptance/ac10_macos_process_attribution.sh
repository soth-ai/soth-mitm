...
# Exclude it from the command-free check; all other process/*.rs must remain
# free of external command invocations.
if command -v rg >/dev/null 2>&1; then
  process_command_refs="$(rg -n 'tokio::process::Command|std::process::Command' crates/soth-mitm/src/process/*.rs | grep -v 'process/macos\.rs:' || true)"
else
  process_command_refs="$(grep -n -E 'tokio::process::Command|std::process::Command' crates/soth-mitm/src/process/*.rs | grep -v 'process/macos\.rs:' || true)"
fi
