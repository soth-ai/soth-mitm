...
ac_run_case "$status_tsv" close_once_unit_contract \
  cargo test --all-features -p soth-mitm --lib stream_end_invokes_connection_close_once -q || true
ac_run_case "$status_tsv" close_stats_finalization_contract \
  cargo test --all-features -p soth-mitm --lib duplicate_stream_end_callbacks_are_deduplicated -q || true
...
ac_run_case "$status_tsv" close_reason_block_path \
  cargo test --all-features -p soth-mitm --test phase_a blocked_host_never_opens_upstream_socket -q || true
ac_run_case "$status_tsv" close_reason_graceful_path \
  cargo test --all-features -p soth-mitm --test phase_a tunnel_action_relays_data_end_to_end -q || true
ac_run_case "$status_tsv" close_reason_downstream_disconnect_path \
  cargo test --all-features -p soth-mitm --lib late_response_after_stream_end_does_not_resurrect_dispatcher -q || true
