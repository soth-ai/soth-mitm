...
ac_run_case "$status_tsv" close_stats_finalization_contract \
  cargo test --features soth-mitm/__internal -p soth-mitm --lib duplicate_stream_end_callbacks_are_deduplicated -q || true
ac_run_case "$status_tsv" close_once_10k_connection_scale \
  env MITM_CORE_CONCURRENCY="$connection_count" \
    cargo test --features soth-mitm/__internal -p soth-mitm --test server_concurrency \
      flow_lifecycle_server_handles_500_parallel_short_lived_connections -q || true
ac_run_case "$status_tsv" close_reason_block_path \
  cargo test --features soth-mitm/__internal -p soth-mitm --test phase_a blocked_host_never_opens_upstream_socket -q || true
ac_run_case "$status_tsv" close_reason_graceful_path \
  cargo test --features soth-mitm/__internal -p soth-mitm --test phase_a tunnel_action_relays_data_end_to_end -q || true
ac_run_case "$status_tsv" close_reason_downstream_disconnect_path \
  cargo test --features soth-mitm/__internal -p soth-mitm --lib late_response_after_stream_end_does_not_resurrect_dispatcher -q || true
