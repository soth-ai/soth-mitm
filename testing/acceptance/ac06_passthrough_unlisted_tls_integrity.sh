...
ac_run_case "$status_tsv" destination_scope_passthrough_contract \
  cargo test --all-features -p soth-mitm destination_scope_intercept_vs_passthrough -q || true
...
ac_run_case "$status_tsv" passthrough_no_tls_mitm \
  cargo test --all-features -p soth-mitm --test phase_a tunnel_action_does_not_emit_tls_handshake_events -q || true
ac_run_case "$status_tsv" http3_passthrough_no_tls_mitm \
  cargo test --all-features -p soth-mitm --test http3_passthrough_mitm \
