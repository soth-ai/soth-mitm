...
ac_run_case "$status_tsv" body_limit_config_validation_contract \
  cargo test --features soth-mitm/__internal -p soth-mitm reject_zero_body_size_limit -q
ac_run_case "$status_tsv" body_limit_runtime_mapping_contract \
  cargo test --features soth-mitm/__internal -p soth-mitm body_size_limit_maps_to_core_runtime_budget -q
ac_run_case "$status_tsv" decoder_budget_clamp_contract \
  cargo test --features soth-mitm/__internal -p soth-mitm decoder_budget_is_clamped_by_body_size_limit -q
ac_run_case_with_socket_permission_fallback "$status_tsv" oversized_sse_decoder_budget_contract "$strict_tools" \
  cargo test --features soth-mitm/__internal -p soth-mitm --test chaos_charter \
