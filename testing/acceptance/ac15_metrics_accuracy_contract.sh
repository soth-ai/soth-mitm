...
ac_run_case "$status_tsv" process_lookup_timeout_contract \
  cargo test --features soth-mitm/__internal -p soth-mitm process_lookup_timeout_sets_none -q
ac_run_case "$status_tsv" process_lookup_cache_contract \
  cargo test --features soth-mitm/__internal -p soth-mitm process_info_resolved_once_per_connection -q
