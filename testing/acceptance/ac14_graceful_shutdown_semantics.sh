...
ac_run_case "$status_tsv" shutdown_noop_when_handle_missing \
  cargo test --all-features -p soth-mitm shutdown_noop_when_handle_already_consumed -q
ac_run_case "$status_tsv" shutdown_drains_active_flows \
  cargo test --all-features -p soth-mitm shutdown_drains_active_flows_before_joining_runtime -q
ac_run_case "$status_tsv" shutdown_timeout_when_flows_stuck \
  cargo test --all-features -p soth-mitm shutdown_returns_timeout_when_active_flows_do_not_drain -q
